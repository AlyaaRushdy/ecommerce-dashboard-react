...
export default function SingleProduct() {
	...
	const getProductFromApi = async () => {
		const productData = await axios.get(`http://localhost:5000/products/${id}`);
		// console.log(productData);
		setProduct(productData.data.body);
	};
	useEffect(() => {
		getProductFromApi();
	}, []);
	const fixImagePath = (imagePath) => {
		return imagePath.replace(
			"I:\\Courses\\web\\DEPI\\project\\tech\\ecommerce-dashboard-backend\\images",
					...
					<div className="grid grid-cols-4 sm:grid-cols-1 gap-4 col-span-2">
						{product.images &&
							product.images.map((img, i) => (
								<Card key={i} className="grid justify-center content-center">
									<img src={fixImagePath(img)} alt="" />
								</Card>
							))}
					</div>
				</section>
